...
viewers.Comments = Class.create({
            ...
            /* Ajax request parameters */
            {
              ...
              onFailure : function (response) {
                ...
                item._x_notification.replace(new XWiki.widgets.Notification(
                    "Failed to retrieve comment:" + failureReason,
                    "error"));
              }.bind(this),
          ...
          new Ajax.Request(url, {
            ...
            onFailure : function (response) {
              ...
              form._x_notification.replace(new XWiki.widgets.Notification(
                  "Failed to post comment:" + failureReason, "error"));
            }.bind(this),
            on0 : function (response) {
              response.request.options.onFailure(response);
            },
            onComplete : function (response) {
              ...
              if (this.requestSucceeded) {
                ...
                form._x_notification.replace(
                  new XWiki.widgets.Notification("Comment posted",
                    "done"));
              }
            }.bind(this)
          });
  ...
  addPreview : function(form) {
    ...
    var previewURL = "/xwiki/bin/preview/__space__/__page__".replace("__space__",
        ...
        new Ajax.Request(previewURL, {
          ...
          onFailure: function (response) {
            ...
            notification.replace(new XWiki.widgets.Notification(
                "Failed to generate preview:" + failureReason, "error"));
          },
          on0: function (response) {
            response.request.options.onFailure(response);
          },
...
require(['jquery', 'xwiki-events-bridge'], function($) {
  ...
  $(document).on('click', '#deleteModal input.btn-danger', function() {
    ...
    $.ajax({
      ...
      success : function() {
        ...
        comment.replaceWith(createNotification("Deleted comment."));
        updateCount();
        notification.replace(new XWiki.widgets.Notification(
          "Comment deleted", 'done'));
  ...
  var updateCount = function() {
    ...
    if(commentsTab) {
      commentsTab.text("(__number__)".replace("__number__", commentsNumber));
    }
    if($('#tmComment').length) {
      // All the sub-nodes of tmComment are added in a normalized form.
      $('#tmComment')[0].normalize();
      var label = " Comments (__number__)";
