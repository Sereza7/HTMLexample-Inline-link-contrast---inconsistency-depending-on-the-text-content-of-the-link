...
this.showDialog();this.setClass("createpage-modal-popup")}});d=$(d||"body").select("span.wikicreatelink:not(.skipCreatePagePopup)");for(var a=0;a<d.length;a++)d[a].down("a").observe("click",function(c){new Ajax.Request(c.findElement("a").href.replace(/#.*$/,""),{method:"get",parameters:{xpage:"createinline",ajax:1},onSuccess:function(e){var g=e.getHeader("redirect");if(g)window.location=g;else{var f=new b.widgets.Notification("Loading...",
"inprogress");g=new Element("link",{rel:"stylesheet",type:"text/css",href:'/xwiki/bin/skin/resources/uicomponents/widgets/select/select.min.css?cache-version=1704187610000'});$(document.head).insert(g);require(["/xwiki/bin/skin/resources/js/xwiki/create.min.js?cache-version=1704187568000","/xwiki/bin/skin/resources/uicomponents/widgets/select/select.min.js?cache-version=1704187606000"],function(h){new b.widgets.CreatePagePopup({content:e.responseText});f.hide()})}},onFailure:function(){(new b.widgets.Notification("An error occurred, please refresh the page and try again",
